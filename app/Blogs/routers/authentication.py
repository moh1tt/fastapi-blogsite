...
from Blogs.token import create_access_token
...
def login(request: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(database.get_db)):
    ...
    if not Hash.verify(user.password, request.password):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Incorrect password for username {request.username}")
